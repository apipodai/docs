...
素材提交后会保存在你的 APIPod 账号下（素材的查询与删除仅限本人提交的素材）。在 Seedance 2.0/2.5 系列视频生成请求中通过 `assetId://{assetId}` 引用素材——`image_url` / `image_urls`、`video_urls`、`audio_url` / `audio_urls` 字段均支持引用图片、视频、音频三类素材。
