...
Protocol-compatible endpoints can preserve their native response shapes. For example, `/v1/models` returns an OpenAI-compatible model list, and authentication can return an OpenAI-compatible `error` object. Clients should use the contract for the endpoint family they call.
