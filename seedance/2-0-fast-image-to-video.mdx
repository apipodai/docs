...
description: "Seedance 2.0 Fast Image to Video is a lower-latency route that animates a required first-frame image and can use an optional last frame where the public schema... Media URLs are submitted to asset review before task creation by default (real_person=true); set real_person=false only when the materials are confirmed face-free."
