...
Poll this endpoint with the task `id` returned by the Ark-compatible create endpoint (`POST /api/v3/contents/generations/tasks`) until the task reaches a terminal state. For production workloads, prefer `callback_url` on the create request over tight polling loops. Task records are retained for the same window as native APIPod tasks; download or persist the video URL promptly once the task succeeds.
