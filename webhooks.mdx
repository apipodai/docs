...
A failed task uses the same envelope, without `usage`:
...
Completed callbacks include a `usage` object with the final amount charged for the task:

| Field | Type | Description |
| --- | --- | --- |
| `usage.cost` | number | Final amount charged for this task, in USD. This is the settled amount after any post-completion adjustment, matching the cost shown in the dashboard. |
| `usage.prompt_tokens` | integer | Present only when the upstream provider reports token usage. |
| `usage.completion_tokens` | integer | Present only when the upstream provider reports token usage. |
| `usage.total_tokens` | integer | Present only when the upstream provider reports token usage. |

`usage` appears only on `completed` callbacks. Failed tasks are refunded per the model's refund policy and carry no usage object.
